...
def main(**args):
    ...
    #python3 smplifyx/main.py --config cfg_files/fit_smpl.yaml --data_folder ./input/ --output_folder ./output --visualize=True --model_folder ./models/ --vposer_ckpt  ../vposer_v1_0 --part_segm_fn smplx_parts_segm.pkl --interpenetration False --use_face False --use_hands False --gender=female
    ...
    for idx, data in enumerate(dataset_obj):
        ...
        for person_id in range(keypoints.shape[0]):
            ...
            curr_meshim_folder = osp.join(output_folder, 'meshim', fn,
                                       '{:03d}'.format(person_id))

            if not osp.exists(curr_img_folder):
                os.makedirs(curr_img_folder)

            if not osp.exists(curr_meshim_folder):
                os.makedirs(curr_meshim_folder)
            ...
            meshim_fn = osp.join(curr_meshim_folder, 'meshin.png')
