...
def fit_single_frame(img,
    ...
    if visualize:
        ...
        output_img_mesh = (color[:, :, :-1] * valid_mask)
        img = pil_img.fromarray((output_img_mesh * 255).astype(np.uint8))
        img.save(meshim_fn)
        print('mesihm saved')

        #scenemesh = as_mesh(scene)
        #scenemesh.export('./scenemesh.obj')
...
def as_mesh(scene_or_mesh):
    ...
    """
    print(type(scene_or_mesh))
    if isinstance(scene_or_mesh, pyrender.scene.Scene):
