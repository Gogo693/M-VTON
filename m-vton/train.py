...
def train_tom(opt, train_loader, model, board):
    ...
    for step in range(opt.keep_step + opt.decay_step):
        ...
        loss_vgg_rend = criterionVGG(p_rendered, im)
        ...
        #Vgg loss
        loss = loss_l1 + loss_vgg + loss_mask + loss_vgg_rend
...
def main():
    ...
    cuda1 = torch.cuda.set_device(opt.gpu_id)
    ...
    input_dim = basic_shape + mesh_dim + pants_dim


    # create model & train & save the final checkpoint
    if opt.stage == 'GMM':
        model = GMM(opt, input_dim)
        ...
        model = UnetGenerator(input_dim + 3, 4, 6, ngf=64, norm_layer=nn.InstanceNorm2d)
        if not opt.checkpoint =='' and os.path.exists(opt.checkpoint):
            load_checkpoint(model, opt.checkpoint)
