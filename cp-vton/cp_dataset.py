...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.stage == 'GMM':
            ...
            if use_mesh == True:
                mesh = Image.open(osp.join(self.data_path, 'meshim', im_name.replace('.jpg', ''), '000', 'meshin.png'))
                mesh = mesh.convert('L')
        ...
        parse_pants = (parse_array == 8).astype(np.float32) + \
        ...
        ppm = torch.from_numpy(parse_pants) # [0,1]

        # upper cloth
        im_c = im * pcm + (1 - pcm) # [-1,1], fill 1 for other parts
        im_h = im * phead - (1 - phead) # [-1,1], fill 0 for other parts
        im_p = im * ppm - (1 - ppm) # [-1,1], fill 0 for other parts
